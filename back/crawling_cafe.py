...
# 카페 정보 파싱 함수
def parse_info_box(info_box, cafe_no=None):
    ...
    if img_list:
        for idx, item in enumerate(img_list.select('div.img_item')):
            ...
            # 파일명 지정: cafe_no_0.jpg 등
            if not cafe_no:
                continue
            filename = f"{sanitize_filename(cafe_no)}_{idx}.jpg"
            local_path = download_image(abs_url, "downloaded_images_cafe", filename)
...
def scrape_structured_info(start_page: int, end_page: int, items_per_page: int = 12):
    ...
    for page in range(start_page, end_page + 1):
        ...
        for item in items[:items_per_page]:  # 🔥 여기서 개수 제한
            ...
            cafe_no_match = re.search(r"tourNo=(\d+)", detail_url)
            cafe_no = cafe_no_match.group(1) if cafe_no_match else None

            parsed = parse_info_box(info_box, cafe_no=cafe_no)
            parsed['cafe_no'] = cafe_no
...
if __name__ == '__main__':
    ...
    with open('chungbuk_cafe.json', 'w', encoding='utf-8') as f:
        json.dump(data, f, ensure_ascii=False, indent=2)

    print(f"총 {len(data)}개의 구조화된 정보가 'chungbuk_cafe.json'에 저장되었습니다.")
