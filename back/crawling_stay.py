...
# 카페 정보 파싱 함수
def parse_info_box(info_box, stay_no=None):
    ...
    if img_list:
        for idx, item in enumerate(img_list.select('div.img_item')):
            ...
            # 파일명 지정: stay_no_0.jpg 등
            if not stay_no:
                continue
            filename = f"{sanitize_filename(stay_no)}_{idx}.jpg"
            local_path = download_image(abs_url, "downloaded_images_stay", filename)
...
def scrape_structured_info(start_page: int, end_page: int, items_per_page: int = 12):
    ...
    for page in range(start_page, end_page + 1):
        ...
        for item in items[:items_per_page]:
            ...
            stay_no_match = re.search(r"tourNo=(\d+)", detail_url)
            stay_no = stay_no_match.group(1) if stay_no_match else None

            parsed = parse_info_box(info_box, stay_no=stay_no)
            parsed['stay_no'] = stay_no
...
if __name__ == '__main__':
    ...
    with open('chungbuk_stay.json', 'w', encoding='utf-8') as f:
        json.dump(data, f, ensure_ascii=False, indent=2)

    print(f"총 {len(data)}개의 구조화된 정보가 'chungbuk_stay.json'에 저장되었습니다.")
